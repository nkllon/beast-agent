...
class EdgeCaseTestAgent(BaseAgent):
    ...
    def __init__(
        self, agent_id: str, capabilities: list, mailbox_url=None, config=None
    ):
...
async def test_startup_mailbox_start_returns_false(mailbox_config, redis_available):
    ...
    agent = EdgeCaseTestAgent(
        agent_id="start-false-agent", capabilities=["test"], mailbox_url=mailbox_config
    )
...
async def test_startup_mailbox_connection_error(mailbox_config, redis_available):
    ...
    invalid_config = MailboxConfig(
        host="invalid-host-does-not-exist", port=6379, db=15, stream_prefix="test"
    )
...
async def test_health_check_message_queue_size(mailbox_config, redis_available):
    ...
    agent = EdgeCaseTestAgent(
        agent_id="queue-size-agent", capabilities=["test"], mailbox_url=mailbox_config
    )
...
async def test_mailbox_message_handler_payload_extraction(
    ...
    agent = EdgeCaseTestAgent(
        agent_id="payload-test-agent", capabilities=["test"], mailbox_url=mailbox_config
    )
