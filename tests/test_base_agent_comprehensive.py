...
async def test_create_mailbox_config_host_only(mailbox_config, redis_available):
    ...
    agent = ComprehensiveTestAgent(
        agent_id="host-only-agent",
        capabilities=["test"],
        mailbox_url="localhost"
    )
...
@pytest.mark.asyncio
@pytest.mark.skipif(not pytest.importorskip("beast_mailbox_core"), reason="beast-mailbox-core not available")
async def test_create_mailbox_config_with_mailboxconfig_instance(mailbox_config, redis_available):
...
async def test_health_check_without_mailbox_config(mailbox_config, redis_available):
    ...
    agent = ComprehensiveTestAgent(
        agent_id="health-no-mailbox-agent",
        capabilities=["test"],
        mailbox_url=None
    )
